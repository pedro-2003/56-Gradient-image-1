...
RECIPES = {
    ...
    "ideogram4":  ["--rank", "16", "--lr", "2e-4", "--include", "layers[.][0-9]+[.](attention|adaln_modulation|feed_forward)", "--no-ckpt", "--train-base", "requant", "--fast-lora", "--screen-passes", "0.5,1,1.5,2,2.5,3,3.5,4,5,6,8,10,12,16"],   # fast-lora + screens on pass counts (I4D 2026-09-26, R1 rehearsal c7460d80): -0.03 % vs without (t -1.8, rule "not worse" met), -16 GB peak (66.4 -> 50.4 GB), -0.23 % vs the champion   # requant: measured -1.03% vs the dequantised base (2026-09-25, af4138aa), closes the gap to the champion   # block layers only: the evaluator re-quantises every patched tensor; the 7 embedder/final-layer tensors alone cost 0.7% (probe_requant 2026-09-24)   # twin released between scores -> fits: 0.68 s/step, peak 64.7 GB (smoke 2026-09-24); the OOM guard re-enables checkpointing if a task needs it
    "z-image":    ["--rank", "16", "--lr", "3e-4"],
    "flux":       ["--rank", "16", "--lr", "3e-4", "--fast-lora", "--no-ckpt", "--screen-passes", "0.5,1,1.5,2,2.5,3,3.5,4,5,6,8,10,12,16"],   # NO --exact-merge: the validator scores on an A100, where Comfy loads an fp8 flux checkpoint as bf16 (unet_dtype: no fp8 compute, 24 GB fits) and merges without stochastic rounding; re-scored A100-style (FA 2026-09-26) the exact-merge arm E2 was +0.85 % vs D (t +15) and lost to the champion by 0.60 %, while D beat it by 0.25 %   # fast-lora + no-ckpt + screens on pass counts: G1 smoke 1.01 s/step, peak 53.5 GB; the OOM guard re-enables checkpointing if a task needs it
